...
window.LESSON = {
  ...
  explore: {
    ...
    onUpdate: (P, Diagram) => {
      ...
      // --------------------------------------------------
      // Enforce Euclid-safe invariants:
      //   A.y < D.y < B.y
      // And ensure the intersection point E lies on BD produced.
      // --------------------------------------------------

      // Keep A strictly above D
      if (A.y >= D.y - 20) A.y = D.y - 20;

      // Keep D strictly above BC
      if (D.y >= B.y - 20) D.y = B.y - 20;

      // Prevent BD from becoming horizontal (rare but keeps math stable)
      if (Math.abs(D.y - B.y) < EPS) D.y = B.y - 20;

      // --------------------------------------------------
      // Compute E = intersection of line BD with horizontal line through A
      // Parametric line: B + t(D-B), solve for y = A.y
      // --------------------------------------------------
      const t = (A.y - B.y) / (D.y - B.y);

      // Enforce "BD produced" i.e., E beyond D, which requires t > 1.
  ...
  steps: [
    {
      ...
      reason: { label:'Exposition + I.Post.1', kind:'constr', tooltip:'Exposition; and Postulate 1: To draw a straight line from any point to any point.' },
      reveal: ['AD'],
